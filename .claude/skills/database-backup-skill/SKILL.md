...
Provides scripts and patterns for creating portable Supabase database backups.

## Backup Script

The backup script is located at: `storage/scripts/backup-db.sh`
...
The script creates:
- `storage/backups/{timestamp}/backup.sql.gz` - Compressed database dump
- `storage/backups/{timestamp}/backup.sh` - Copy of backup script
- `storage/backups/{timestamp}/restore.sh` - Restore script for this backup
- `storage/backups/{timestamp}/metadata.json` - Backup metadata

## Restore Script

The restore script is automatically created in each backup directory by the backup script. See `database-restore-skill` for restore operations.
