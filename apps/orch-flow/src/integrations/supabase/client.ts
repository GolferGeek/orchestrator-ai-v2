...
const SUPABASE_API_PORT =
  import.meta.env.VITE_SUPABASE_API_PORT ||
  import.meta.env.SUPABASE_API_PORT ||
  "6010";
const SUPABASE_URL =
  import.meta.env.VITE_SUPABASE_URL || `http://127.0.0.1:${SUPABASE_API_PORT}`;
...
export async function isSupabaseAvailable(): Promise<boolean> {
  ...
  if (supabaseAvailabilityCache) {
    const now = Date.now();
    if (
      now - supabaseAvailabilityCache.checkedAt <
...
if (!SUPABASE_URL || !SUPABASE_PUBLISHABLE_KEY) {
  ...
  console.error(
    ...
    SUPABASE_PUBLISHABLE_KEY ? "SET" : "UNDEFINED",
  );
  console.error(
    "  All VITE_ vars:",
...
export const supabase = createClient<Database>(
  ...
  SUPABASE_PUBLISHABLE_KEY,
  {
    ...
    global: {
      headers: {
        "Accept-Profile": "orch_flow", // Tell PostgREST to use orch_flow schema
        "Content-Profile": "orch_flow", // For INSERT/UPDATE/DELETE operations
      },
