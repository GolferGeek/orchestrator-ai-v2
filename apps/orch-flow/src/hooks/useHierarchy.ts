...
export function useHierarchy(organizationSlug?: string | null) {
  ...
  const fetchAll = useCallback(async () => {
    console.log("[fetchAll] Starting... teamId:", organizationSlug);

    // Query efforts - filter by team if teamId is provided
    let effortsQuery = supabase.from("efforts").select("*");

    if (organizationSlug) {
      // Filter by team_id (organizationSlug is actually teamId in this context)
      effortsQuery = effortsQuery.eq("team_id", organizationSlug);
    }

    const effortsRes = await effortsQuery.order("order_index");
  ...
  useEffect(() => {
    ...
    isSupabaseAvailable().then((isAvailable) => {
      if (!isAvailable) {
        ...
        console.debug(
          "[useHierarchy] Supabase not accessible - skipping realtime subscriptions",
        );
        return;
      }
      ...
      try {
        effortChannel = supabase
          ...
          .on(
            "postgres_changes",
            { event: "*", schema: "orch_flow", table: "efforts" },
            fetchAll,
          )
          .subscribe((status) => {
            if (status === "SUBSCRIBED") {
              ...
              console.warn(
                "⚠️ Could not subscribe to efforts changes (Supabase may not be accessible)",
              );
            }
          });
        ...
        goalChannel = supabase
          ...
          .on(
            "postgres_changes",
            { event: "*", schema: "orch_flow", table: "goals" },
            fetchAll,
          )
          .subscribe((status) => {
            if (status === "SUBSCRIBED") {
              ...
              console.warn(
                "⚠️ Could not subscribe to goals changes (Supabase may not be accessible)",
              );
            }
          });
        ...
        projectChannel = supabase
          ...
          .on(
            "postgres_changes",
            { event: "*", schema: "orch_flow", table: "projects" },
            fetchAll,
          )
          .subscribe((status) => {
            if (status === "SUBSCRIBED") {
              ...
              console.warn(
                "⚠️ Could not subscribe to projects changes (Supabase may not be accessible)",
              );
            }
          });
      } catch (error) {
        console.warn(
          "⚠️ Could not set up realtime subscriptions (Supabase may not be accessible):",
          error,
        );
      }
    });
  ...
  const addEffort = async (name: string) => {
    ...
    // Efforts are now team-scoped (organization_slug = NULL)
    console.log("[addEffort] Inserting team-scoped effort");
    const { data, error } = await supabase
      .from("efforts")
      .insert({
        name,
        organization_slug: null, // Team-scoped
        order_index: 0,
      })
      .select()
      .single();
  ...
  const addGoal = async (effortId: string, name: string) => {
    ...
    const { data, error } = await supabase
      .from("projects")
      .insert({ effort_id: effortId, name, order_index: 0 })
      .select()
      .single();
    if (!error) fetchAll();
    return { data, error };
