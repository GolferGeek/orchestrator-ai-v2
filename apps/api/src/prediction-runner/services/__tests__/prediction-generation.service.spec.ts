...
describe('PredictionGenerationService', () => {
  ...
  describe('attemptPredictionGeneration', () => {
    ...
    it('should return existing prediction if analyst prediction exists and no refresh needed', async () => {
      ...
      predictorManagementService.evaluateThreshold.mockResolvedValue({
        ...mockThresholdResult,
        meetsThreshold: false, // No new threshold met
      });
      ...
      expect(result).toEqual(existingPrediction);
    });

    it('should generate new prediction when threshold is met and no existing prediction', async () => {
      predictionRepository.findByTarget.mockResolvedValue([]);
      predictorManagementService.evaluateThreshold.mockResolvedValue(
        mockThresholdResult,
      );
      ...
      portfolioRepository.getAllCurrentAnalystContextVersions.mockResolvedValue(
        new Map(),
      );
  ...
  describe('generatePrediction', () => {
    it('should create analyst predictions with three-way fork assessments', async () => {
      ...
      predictionRepository.create.mockImplementation(async (data) => ({
        ...mockPrediction,
        ...data,
    ...
    it('should consume predictors after creating prediction', async () => {
      targetService.findByIdOrThrow.mockResolvedValue(mockTarget);
      predictorManagementService.getActivePredictors.mockResolvedValue([
        mockPredictor,
      ]);
      ...
      expect(predictorManagementService.consumePredictors).toHaveBeenCalledWith(
        'target-123',
        expect.any(String),
      );
    });

    it('should create snapshot for audit trail', async () => {
      targetService.findByIdOrThrow.mockResolvedValue(mockTarget);
      predictorManagementService.getActivePredictors.mockResolvedValue([
        mockPredictor,
      ]);
      ...
      expect(snapshotService.buildSnapshotData).toHaveBeenCalled();
      expect(snapshotService.createSnapshot).toHaveBeenCalled();
    });
  });
  ...
  describe('direction mapping', () => {
    ...
    it('should map bearish to down correctly', async () => {
      ...
      targetService.findByIdOrThrow.mockResolvedValue(mockTarget);
      predictorManagementService.getActivePredictors.mockResolvedValue([
        { ...mockPredictor, direction: 'bearish' },
      ]);
      const bearishAssessment = {
    ...
    it('should map neutral to flat correctly', async () => {
      ...
      targetService.findByIdOrThrow.mockResolvedValue(mockTarget);
      predictorManagementService.getActivePredictors.mockResolvedValue([
        { ...mockPredictor, direction: 'neutral' },
      ]);
  ...
  describe('magnitude categorization', () => {
    it('should categorize small magnitude correctly', async () => {
      predictionRepository.findByTarget.mockResolvedValue([]);
      predictorManagementService.evaluateThreshold.mockResolvedValue(
        mockThresholdResult,
      );
      targetService.findByIdOrThrow.mockResolvedValue(mockTarget);
      // Low strength predictors = small magnitude
      predictorManagementService.getActivePredictors.mockResolvedValue([
        { ...mockPredictor, strength: 2 },
      ]);
