...
\restrict nxTJ1NEy9PR2nsQficIrhKyESapywxtMjq1bwd0U2wQ7VoAhoAqXHztXApWGLQJ
...
CREATE TABLE public.agents (
    ...
    created_at timestamp with time zone DEFAULT now() NOT NULL,
    updated_at timestamp with time zone DEFAULT now() NOT NULL,
    CONSTRAINT agents_agent_type_check CHECK ((agent_type = ANY (ARRAY['context'::text, 'api'::text, 'external'::text, 'rag-runner'::text, 'orchestrator'::text]))),
    CONSTRAINT agents_api_has_endpoint CHECK (((agent_type = ANY (ARRAY['context'::text, 'rag-runner'::text, 'orchestrator'::text])) OR (endpoint IS NOT NULL))),
    CONSTRAINT agents_api_no_llm CHECK (((agent_type = ANY (ARRAY['context'::text, 'rag-runner'::text, 'orchestrator'::text])) OR (llm_config IS NULL))),
    CONSTRAINT agents_context_has_llm CHECK (((agent_type <> ALL (ARRAY['context'::text, 'rag-runner'::text])) OR (llm_config IS NOT NULL))),
    CONSTRAINT agents_context_no_endpoint CHECK (((agent_type <> ALL (ARRAY['context'::text, 'rag-runner'::text])) OR (endpoint IS NULL)))
);
...
\unrestrict nxTJ1NEy9PR2nsQficIrhKyESapywxtMjq1bwd0U2wQ7VoAhoAqXHztXApWGLQJ
