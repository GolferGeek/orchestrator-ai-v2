...
const _LLM_TIMEOUT = 120000; // 2 minutes
...
describeE2E("Marketing Swarm E2E Tests", () => {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const _observabilityEvents: any[] = [];
  ...
  const llmConfigs = {
    ...
    localWriter2: {
      agentSlug: "writer-conversational",
      llmProvider: "ollama",
      llmModel: "llama3.2",
    },
  };
  ...
  async function cleanupTestData(taskId: string): Promise<void> {
    // Delete evaluations
    await marketingSupabase.from("evaluations").delete().eq("task_id", taskId);
   ...
   */
  function getData(
    body: { data?: unknown; success?: boolean } | unknown,
  ): unknown {
  ...
  describe("1. Database Setup and Task Creation", () => {
    ...
    it("should have static LLM configs for local and cloud providers", async () => {
      ...
      const localConfigs = Object.values(llmConfigs).filter(
        (c) => c.llmProvider === "ollama",
      );
    ...
    it("should create a task with proper config structure", async () => {
      ...
      await createTestTask({
        taskId,
        writers: [llmConfigs.cloudWriter],
  ...
  describe("2. Basic Execution Flow (Cloud Only)", () => {
    ...
    it(
      "should execute the swarm and return results",
      async () => {
  ...
  describe("3. Dual-Track Execution", () => {
    ...
    it(
      "should process local and cloud outputs separately",
      async () => {
        const context = createTestContext(taskId);
        ...
        expect([200, 201]).toContain(response.status);
        const _data = getData(response.body) as any;
        ...
        const localOutputs = outputs!.filter(
          (o: any) => o.writer_llm_provider === "ollama",
        );
        const cloudOutputs = outputs!.filter(
          (o: any) => o.writer_llm_provider !== "ollama",
        );
  ...
  describe("4. Edit Cycle Loop", () => {
    ...
    it(
      "should track edit cycles correctly",
      async () => {
        const context = createTestContext(taskId);
  ...
  describe("5. Two-Stage Evaluation", () => {
    ...
    it(
      "should complete initial evaluation with 1-10 scores",
      async () => {
        const context = createTestContext(taskId);
    ...
    it("should calculate initial rankings and select finalists", async () => {
      ...
      const rankedOutputs = outputs!.filter((o) => o.initial_rank !== null);
        ...
        for (const evaluation of finalEvals!) {
          ...
          if (evaluation.status === "completed") {
            ...
            if (evaluation.weighted_score !== null) {
              expect([100, 60, 30, 10, 5, 0]).toContain(
    ...
    it("should calculate final rankings with weighted scores", async () => {
      ...
      const rankedFinalists =
        outputs?.filter((o) => o.final_rank !== null) || [];
  ...
  describe("7. SSE/Observability Messages", () => {
    it(
      "should emit events with correct metadata structure",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
    ...
    it(
      "should include cost data in output_updated SSE events",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
    ...
    it(
      "should include cost data in evaluation_updated SSE events",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
        ...
        for (const evaluation of evaluations!) {
          ...
          console.log(
            `Evaluation ${evaluation.id} (${evaluation.stage}) cost data for SSE:`,
            {
              cost: metadata.cost,
              tokensUsed: metadata.tokensUsed,
  ...
  describe("8. Edge Cases and Error Handling", () => {
    ...
    it(
      "should handle concurrent execution requests",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
  ...
  describe("9. Output Matrix Validation", () => {
    it(
      "should create correct number of outputs (writers × editors)",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
  ...
  describe("11. LLM Metadata Tracking", () => {
    it(
      "should store LLM metadata for outputs",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
    ...
    it(
      "should store LLM metadata for evaluations",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
  ...
  describe("12. LLM Cost Tracking", () => {
     ...
     */
    it(
      "should store cost in outputs table after writing",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
     ...
     */
    it(
      "should accumulate costs across edit cycles",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
        ...
        for (const output of outputs!) {
          ...
          if (editCycle > 0) {
            ...
            expect(metadata.llmCallCount as number).toBeGreaterThanOrEqual(
              1 + editCycle,
            );
          }
     ...
     */
    it(
      "should store cost in evaluations table",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
     ...
     */
    it(
      "should add evaluation costs to output running total",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
     ...
     */
    it(
      "should store costs in output_versions for each write/rewrite",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
     ...
     */
    it(
      "should calculate different costs for different providers/models",
      async () => {
        const taskId = uuidv4();
        ...
        const { data: outputs } = await marketingSupabase
          .from("outputs")
          .select(
            "id, writer_agent_slug, writer_llm_provider, writer_llm_model, llm_metadata",
          )
          .eq("task_id", taskId);
        ...
        if (outputs!.length > 1) {
          const costs = outputs!.map(
            (o) => (o.llm_metadata as Record<string, unknown>).cost as number,
          );
     ...
     */
    it(
      "should allow calculation of total task cost from all outputs",
      async () => {
        const taskId = uuidv4();
        // Using static llmConfigs instead of database lookups
        ...
        console.log(
          `  Total from outputs (includes eval): $${totalOutputCost.toFixed(4)}`,
        );
