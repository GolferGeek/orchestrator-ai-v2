...
let DatabaseService = DatabaseService_1 = class DatabaseService {
    ...
    async getFilterOptions() {
        const queries = await Promise.all([
            this.pool.query('SELECT DISTINCT source_app FROM observability.events ORDER BY source_app'),
            this.pool.query('SELECT DISTINCT session_id FROM observability.events ORDER BY session_id DESC LIMIT 300'),
            this.pool.query('SELECT DISTINCT hook_event_type FROM observability.events ORDER BY hook_event_type'),
        ]);
        return {
