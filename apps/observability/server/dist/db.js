...
exports.updateEventHITLResponse = updateEventHITLResponse;
exports.insertTheme = insertTheme;
exports.updateTheme = updateTheme;
exports.getTheme = getTheme;
exports.getThemes = getThemes;
exports.deleteTheme = deleteTheme;
exports.incrementThemeDownloadCount = incrementThemeDownloadCount;
...
async function getFilterOptions() {
    const queries = await Promise.all([
        pool.query('SELECT DISTINCT source_app FROM observability.events ORDER BY source_app'),
        pool.query('SELECT DISTINCT session_id FROM observability.events ORDER BY session_id DESC LIMIT 300'),
        pool.query('SELECT DISTINCT hook_event_type FROM observability.events ORDER BY hook_event_type'),
    ]);
    return {
...
async function getRecentEvents(limit = 300) {
    const query = `
  ...
  `;
    ...
    return result.rows.map((row) => ({
        ...
        chat: row.chat ? (typeof row.chat === 'string' ? JSON.parse(row.chat) : row.chat) : undefined,
        summary: row.summary || undefined,
        timestamp: row.timestamp,
        humanInTheLoop: row.human_in_the_loop ? (typeof row.human_in_the_loop === 'string' ? JSON.parse(row.human_in_the_loop) : row.human_in_the_loop) : undefined,
        humanInTheLoopStatus: row.human_in_the_loop_status ? (typeof row.human_in_the_loop_status === 'string' ? JSON.parse(row.human_in_the_loop_status) : row.human_in_the_loop_status) : undefined,
        model_name: row.model_name || undefined,
    })).reverse();
}
async function updateEventHITLResponse(id, response) {
    const status = {
        status: 'responded',
    ...
    RETURNING *
  `;
    const result = await pool.query(query, [JSON.stringify(status), id]);
...
async function incrementThemeDownloadCount(id) {
    const query = `
    UPDATE observability.themes
