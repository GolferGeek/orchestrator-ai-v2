#!/usr/bin/env node
/**
 * Notebook Onboarding Script - Small Team (With Developers)
 * 
 * Creates comprehensive Notebook content (notebooks/documents) for small teams with developers
 * Includes both business and technical documentation
 */
...
async function seedNotebookSmallWithDevs(orgSlug) {
...
if (!orgSlug) {
  console.error('❌ Usage: node seed-notebook-small-with-devs.js <organization-slug>');
  console.error('   Example: node seed-notebook-small-with-devs.js demo-org');
  process.exit(1);
}

seedNotebookSmallWithDevs(orgSlug).catch((error) => {
